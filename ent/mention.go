...
func (*Mention) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case mention.FieldLocal:
			values[i] = new(sql.NullBool)
		case mention.FieldID, mention.FieldStatusID, mention.FieldProfileID:
			values[i] = new(sql.NullInt64)
		case mention.FieldCreatedAt, mention.FieldUpdatedAt, mention.FieldDeletedAt:
...
func (_m *Mention) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case mention.FieldCreatedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field created_at", values[i])
			} else if value.Valid {
				_m.CreatedAt = value.Time
			}
		case mention.FieldUpdatedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field updated_at", values[i])
			} else if value.Valid {
				_m.UpdatedAt = value.Time
			}
		case mention.FieldDeletedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
