...
func (_q *StoryQuery) QueryProfile() *ProfileQuery {
...
func (_q *StoryQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *StoryQuery) FirstIDX(ctx context.Context) uint64 {
...
func (_q *StoryQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *StoryQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (_q *StoryQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (_q *StoryQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *StoryQuery) WithProfile(opts ...func(*ProfileQuery)) *StoryQuery {
	query := (&ProfileClient{config: _q.config}).Query()
...
func (_q *StoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Story, error) {
	...
	if query := _q.withProfile; query != nil {
		if err := _q.loadProfile(ctx, query, nodes, nil,
			func(n *Story, e *Profile) { n.Edges.Profile = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *StoryQuery) loadProfile(ctx context.Context, query *ProfileQuery, nodes []*Story, init func(*Story), assign func(*Story, *Profile)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Story)
	for i := range nodes {
...
func (_q *StoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(story.Table, story.Columns, sqlgraph.NewFieldSpec(story.FieldID, field.TypeUint64))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProfile != nil {
			_spec.Node.AddColumnOnce(story.FieldProfileID)
		}
	}
